...
So far what we were doing was reanalysis. So we will need to reproduce climatilogical features of reanalysis fields in the past

This time our focus will not be globally but only over a selected grid

After that we are going to run the same analysis we run on the previous excertation


After that we are going to consinder any other data sets which can be downloaded from the Copernicus Climate Data Store
...
```
which are responsible for raeding ncdf files and for finding leap years respetively
...
for(i in 1:n_year){
  ...
  if(i==1){
    ...
    if(year_leap[i]==FALSE) day_sequence[,3] <- day #If the year is not leap assighn it to variable day
    if(year_leap[i]==TRUE) day_sequence[,3] <- day_leap #If the year is leap assighn it to variable day_leap
...
This data contains all the possible times from the beginning to the end my it 
is not mandatory that all these times are present in our data

we create a vector names time_ok. If zou just run the variable **time** you can 
see all possible times. From that varibale we define our new vector time_ok. 
...
generally we are taking time_ok data from the 3rd column of the varibale *day_sequence*
...
This matrix has data for all availlable grid points in time okay. But we need to
work on a specific grid. In class we selected france. To know the right lat and 
lon, we need to go to google earth.


If you have a donloaded version of gooogle earth search an area  in our case 
france.Turn on the grids by going to the view > check grids. i have highlighted 
...
We now have data but instead of global the data is for a given area
we can now run analysis on the data  just as we ddi from our earlier lessons 
```{R}
knitr::purl()
```
